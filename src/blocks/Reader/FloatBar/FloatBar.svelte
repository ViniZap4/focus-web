<script lang="ts">
	...
	function resetHide() {
		...
		hideTimer = setTimeout(() => {
			if (reader.isPlaying && !showPanel && !reader.showSections) visible = false;
		}, 3000);
	}
...
<div class="bar" class:hidden={!visible}>
	<div class="prog">
		<div class="prog-fill" style="width:{reader.progress}%"></div>
	</div>
				...
				<div class="p-grid-2">
					<div class="sl-group">
						<div class="sl-head">
							...
							<span class="sl-val">{reader.settings.fontSize}px</span>
						</div>
						<input type="range" min="20" max="64" step="2" bind:value={reader.settings.fontSize} oninput={save} />
					</div>
					<div class="sl-group">
						<div class="sl-head">
							<span class="sl-label">Line height</span>
							<span class="sl-val">{reader.settings.lineHeight.toFixed(1)}</span>
						</div>
				...
				<div class="p-grid-2">
					<div class="sl-group">
						<div class="sl-head">
							...
							<span class="sl-val">{reader.settings.wpm} wpm</span>
						</div>
						<input type="range" min="60" max="600" step="10" bind:value={reader.settings.wpm} oninput={save} />
					</div>
					<div class="sl-group">
						<div class="sl-head">
							<span class="sl-label">Pitch</span>
							<span class="sl-val">{reader.settings.speechPitch.toFixed(1)}</span>
						</div>
						<input type="range" min="0.5" max="2" step="0.1" bind:value={reader.settings.speechPitch} oninput={save} />
					</div>
				</div>
			</div>
...
<style>
	...
	.prog { height: 2px; background: var(--surface); }
	.prog-fill {
		height: 100%;
		background: var(--text-4);
		transition: width 0.4s var(--ease), background var(--dur-slow) var(--ease);
	}
