<script lang="ts">
	...
	function resumeReading(session: ReadingSession) {
		if (reader.resumeSession(session)) {
			goto('/reader');
		} else {
			error = 'Could not resume — text no longer available. Try reopening the file.';
		}
	}
	...
	async function handleFile(file: File) {
		...
		parseProgress = 0;
		try {
			const result = await parseFile(file, (pct) => { parseProgress = pct; });
...
>
	...
	<div class="input-bar stagger" style="transition-delay:100ms">
		...
		<button class="go-btn" onclick={start} disabled={!customText.trim() || parsing} title="Start reading">
	...
	{#if wordCount > 0}
		<span class="word-count">{wordCount.toLocaleString()} words · ~{readTime} min</span>
	{/if}
	...
	<div class="samples stagger" style="transition-delay:200ms">
		{#each sampleTexts as sample, i}
			<button class="sample stagger" style="transition-delay:{240 + i * 40}ms" onclick={() => select(sample)}>
...
<style>
	...
	.home.drag-over { background: var(--surface); }
	...
	.ready .stagger { opacity: 1; transform: translateY(0) scale(1); }
	...
	.input-bar {
		...
		width: 100%;
		max-width: 520px;
		transition: border-color var(--dur) var(--ease), background-color var(--dur-slow) var(--ease), box-shadow var(--dur) var(--ease);
	}
	.input-bar:focus-within { border-color: var(--border-h); box-shadow: var(--shadow-sm); }

	.upload-btn {
		all: unset; cursor: pointer;
		display: flex; align-items: center; gap: 0.35rem;
		padding: 0.5rem 0.7rem; border-radius: 12px;
		background: var(--surface-h); color: var(--text-2);
		font-size: 0.8rem; font-weight: 500; white-space: nowrap; flex-shrink: 0;
		transition: all var(--dur) var(--ease);
	}
	.upload-btn:hover:not(:disabled) { background: var(--surface-a); color: var(--text); transform: scale(1.02); }
	...
	.input-wrap textarea {
		...
		padding: 0.5rem 0.6rem; font-size: 0.9rem; color: var(--text);
		line-height: 1.5; resize: none; overflow: hidden;
		height: 36px; max-height: 180px; box-sizing: border-box;
		transition: color var(--dur-slow) var(--ease);
	}
	...
	.go-btn {
		...
		border-radius: 10px; background: var(--surface-h); color: var(--text-2);
		flex-shrink: 0; transition: all var(--dur) var(--ease);
	}
	.go-btn:hover:not(:disabled) { background: var(--surface-a); color: var(--text); transform: scale(1.05); }
	.go-btn:active:not(:disabled) { transform: scale(0.93); }
	.go-btn:disabled { opacity: 0.12; cursor: not-allowed; }

	.word-count { font-size: 0.65rem; color: var(--text-4); font-variant-numeric: tabular-nums; }
	...
	.hi-time { font-size: 0.6rem; color: var(--text-4); flex-shrink: 0; }
	...
	.hi-bottom { display: flex; align-items: center; gap: 0.5rem; }
	.hi-bar { flex: 1; height: 3px; border-radius: 2px; background: var(--surface-h); }
	.hi-fill { height: 100%; border-radius: 2px; background: var(--text-3); transition: width var(--dur) var(--ease); }
	.hi-pct { font-size: 0.6rem; color: var(--text-4); font-variant-numeric: tabular-nums; min-width: 2rem; text-align: right; }
	...
	.sample {
		...
		font-size: 0.78rem; transition: all var(--dur) var(--ease);
	}
	.sample:hover { color: var(--text-2); border-color: var(--border-h); background: var(--surface); transform: translateY(-2px); }
	...
	.parsing { display: flex; align-items: center; gap: 0.5rem; color: var(--text-3); font-size: 0.8rem; }
